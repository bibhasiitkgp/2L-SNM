...
load('elec.mat'); %soc-sign-bitcoinalpha
n=length(A);
for i=1:n
    for j=1:n
        if(A(i,j)~=0)
...
xyz0=sum(tri4)/(sum(tri1)+sum(tri2)+sum(tri3)+sum(tri4));
      ...
      error=sum(abs(log(cumdegree)-log(cumdegree1(1:length(cumdegree)))))/n+sum(log(abs(cumdegree1(1+length(cumdegree):length(cumdegree1)))))/n+abs((t3-sum(tri1))/t3)+abs(xyz3-T3data)/T3data+abs(xyz2-T2data)/T2data+abs(xyz1-T1data)/T1data+abs(xyz0-T0data)/T0data;
        end
