...
%     end
% end
% Elist=zeros(length(find(A~=0))/2,4);
% count=1;
% for i=1:n
%     for j=i+1:n
%         if(A(i,j)~=0)
...
for i=1:length(x)
    ...
    for j=1:length(x)
    p=x(j);
   c11=a11*beta/2+(1-beta)*p*(sum(PosDegree)/(sum(PosDegree)+sum(NegDegree)))+p*epsilon/(1-epsilon);
   c22=a22*beta/2+(1-beta)*(sum(PosDegree)/(sum(PosDegree)+sum(NegDegree)))*p*(sum(NegDegree)/sum(PosDegree));
   c12=a12*beta/2+(1-beta)*p*(sum(PosDegree)/(sum(PosDegree)+sum(NegDegree)))*(sum(NegDegree)/sum(PosDegree))+epsilon/(1-epsilon);
   c21=a21*beta/2+(1-beta)*(1-sum(PosDegree)/sum(NegDegree))*p*sum(NegDegree)/sum(PosDegree)+p*epsilon/(1-epsilon)+(sum(PosDegree)/(sum(PosDegree)+sum(NegDegree)))/(1-(sum(PosDegree)/(sum(PosDegree)+sum(NegDegree))));
...
for t1=1:1
beta=x2(t43);
epsilon=0.61*(t2+t0)/(t3+t4+t0+t2);
a11=1;
a12=alpha(2,1)*a11;
a21=alpha(2,1)*a22;
